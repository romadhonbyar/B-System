...
class Ion_auth_acl_model extends Ion_auth_model
{
    ...
    function __construct()
    {
        ...
        //$this->lang->load('main_auth/ion_auth_acl');
		...
		$this->messages    = array();
		$this->errors      = array();
		$delimiters_source = $this->config->item('delimiters_source', 'ion_auth_acl');
		...
		else
		{
			...
			$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'ion_auth_acl');
			$this->message_end_delimiter   = $this->config->item('message_end_delimiter', 'ion_auth_acl');
			$this->error_start_delimiter   = $this->config->item('error_start_delimiter', 'ion_auth_acl');
			$this->error_end_delimiter     = $this->config->item('error_end_delimiter', 'ion_auth_acl');
		}
    ...
    public function add_permission_to_user($group_id = FALSE, $perm_id = FALSE, $value = 0)
    {
        ...
        if ($this->db->trans_status() === FALSE)
        {
            $this->set_error('user_permission_add_unsuccessful');
            return FALSE;
        }
        else
        {
